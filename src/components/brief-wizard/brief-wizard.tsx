...
import { Step6Meeting } from "./step-6-meeting";
...
export function BriefWizard({
  ...
  const toggleArray = <K extends "goals" | "services">(
    key: K,
    value: BriefData[K][number],
  ) =>
    setData((d) => {
  ...
  const canAdvance = [
    ...
    data.contactMethod !== null &&
      (data.contactMethod !== "meet" || data.meetingDate !== null),
  ][step];
  ...
  return (
    <div className="grid gap-8">
      ...
      <div className="min-h-[360px]">
        ...
        {step === 1 && (
          <Step1Goals data={data} toggle={(v) => toggleArray("goals", v)} />
        )}
        {step === 2 && (
          <Step2Services
            data={data}
            toggle={(v) => toggleArray("services", v)}
          />
        )}
        ...
        {step === 6 && <Step6Meeting data={data} update={update} />}
      </div>
      <WizardNav
        step={step}
